...
const Input = ({ label, type }) => {
   return (
      <div>
         <label form='' className='text-lg text-black'>{label}</label>
         <input type={type} className='ring-1 outline-none mt-1 ring-gray-300 px-4 py-2 min-w-[250px] w-full'></input>
      </div>
   )
}
            ...
            >
               <div className="flex flex-col gap-7">
                  <div className="flex gap-4">
                     ...
                     <Details title='Location:' text='A108 Adam Street, New York, NY 535022' />
                  </div>
                  <div className="flex gap-4">
                     <div className="flex items-center justify-center p-4 rounded-full bg-[#dff3fc] text-[#149ddd]">
                        <BiEnvelope size={23} />
                     </div>
                     <Details title='Email:' text='info@example.com' />
                  </div>
                  <div className="flex gap-4">
                     <div className="flex items-center justify-center p-4 rounded-full bg-[#dff3fc] text-[#149ddd]">
                        <BsPhone size={23} />
                     </div>
                     <Details title='Call:' text='+1 5589 55488 55s' />
                  </div>
               </div>

               <div className="mt-8 flex flex-col gap-1 items-start">
                  <div className="bg-[#2C2F3F] w-32 h-32 p-2 rounded-full">
                     <img
                        className="w-full h-full rounded-full"
                        src="./images/profile.jpg"
                        alt="profile-img" />
                  </div>
                  <p className="text-2xl font-semibold my-[14px]">Fake Person</p>
